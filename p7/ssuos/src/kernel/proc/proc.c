...
void lseek_proc(void *aux , void *filename)
{
	...
	else
	{
		if (!strcmp(aux, "-e"))
			opt |= OPT_E;
		else if (!strcmp(aux, "-re"))
			opt |= OPT_RE;
		else if (!strcmp(aux, "-a"))
			opt |= OPT_A;
		else if (!strcmp(aux, "-c"))
			opt |= OPT_C;
		else
		{
	...
	switch (opt)
	{
		...
		case OPT_E:
			printk("\n<Option e test>\n");
			fd = open("opt_e_test", O_RDWR);
			if (fd<0)return;
			write(fd, "opt_e", 5);
			lseek(fd, 7, SEEK_SET|opt);
